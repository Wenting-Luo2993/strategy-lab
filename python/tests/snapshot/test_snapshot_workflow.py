...
def test_snapshot_workflow_create_update(assert_snapshot, monkeypatch, tmp_path):
    ...
    df = pd.DataFrame({"timestamp": [1,2,3], "entry_signal": [0, 1, 0], "exit_flag": [0, 0, 1]}).set_index("timestamp")
    ...
    # Modify entry_signal to induce diff
    df2 = pd.DataFrame({"timestamp": [1,2,3], "entry_signal": [0, -1, 0], "exit_flag": [0, 0, 1]}).set_index("timestamp")
