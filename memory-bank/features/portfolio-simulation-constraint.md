...
| B2 | **No buying-power check (hard blocker)** | `_position_size` (`engine.py:387`) returns `max(1, int(risk_dollars / stop_distance))` with no cash bound, and `Portfolio` moves cash without a floor. **Now measured, not theoretical**: a single-symbol ORB run on QQQ 2019-2023 (1256 trades) reached `min_cash` of **-$26,657,576** and `max_gross_exposure_ratio` of **10.82** against $100k of capital. A portfolio backtest on top of this is **meaningless, not merely inaccurate**. Two distinct fixes are required: (a) enforce buying power — available today via `ExecutionRealismConfig.realistic()`, though it currently rejects `orb_production` outright; (b) make `_position_size` honour `PositionSizeConfig.max_shares` / `max_position_pct`, which it ignores entirely even though the **live bot's `PositionSizer` enforces both**. Note `orb_production.yaml` sets neither cap, so the ruleset needs updating too. |
